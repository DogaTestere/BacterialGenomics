...
workflow BACTMAP {
    ...
    PROKKA ( VCF2PSEUDOGENOME.out.pseudogenome )
...
process PROKKA {
    tag "$meta.id"
    label 'process_low'
    publishDir "${params.outdir}/anotasyon_sonuclari", mode: 'copy'

    input:
    tuple val(meta), path(fasta)

    output:
    path "${meta.id}/*", emit: prokka_files

    script:
    """
    prokka --outdir ${meta.id} --prefix ${meta.id} ${fasta} --cpus 2
    """
}
